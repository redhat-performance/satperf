...
function measure() {
    ...
    a $concurency-backup-used-containers-count.log -m shell -a "touch /root/container-used-count; cp /root/container-used-count{,.foobarbaz}" docker-hosts
    reg_five $host_fives
    ...
    log "$( ./reg-average.sh RegisterPuppet $logs/$concurency-RegisterPuppet.log | tail -n 1 )"
    s $sleep_time

    ap $concurency-PickupPuppetOne.log playbooks/tests/puppet-big-test.yaml --tags DEPLOY_SINGLE -e "size=$concurency"
    log "$( ./reg-average.sh PickupPuppet $logs/$concurency-PickupPuppetOne.log | tail -n 1 )"
    s $sleep_time
}
...
function measure_lots() {
    ...
    log "$( ./reg-average.sh PickupPuppet $logs/$concurency-PickupPuppetBunch.log | tail -n 1 )"
    s $sleep_time
}
