...
needed_concurrent_registrations_per_container_host="$(( ( expected_concurrent_registrations + number_container_hosts - 1 ) / number_container_hosts / 4 * 4 < 4 ? 4 : ( expected_concurrent_registrations + number_container_hosts - 1 ) / number_container_hosts / 4 * 4 ))" # We want ceiling rounding: Ceiling( X / Y ) = ( X + Y – 1 ) / Y, then rounded to multiple of 4
