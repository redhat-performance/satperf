...
###a $X/rex-date-40.log -m "shell" -a "hammer -u admin -p changeme job-invocation create --description-format 'Run %{command} (%{template_name})' --description-format 'Date with 40 ($d)' --dynamic --search-query 'container' --job-template 'Run Command - SSH Default' --inputs command='date' --async" satellite6 &
###s 30
