...
for row in $( cut -d ' ' -f 1 $tmp ); do
    ...
    h_out "--no-headers --csv hostgroup list --search 'name = $hostgroup_name'" | grep --quiet '^[0-9]\+,' \
        || ap regs-41-hostgroup-create-$capsule_name.log playbooks/satellite/hostgroup-create.yaml -e "organization='{{ sat_org }}' hostgroup_name=$hostgroup_name subnet_name=$subnet_name"
done
