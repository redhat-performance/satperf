#!/bin/bash
# Non-FAM section functions — sourced by full_fam.sh and other experiment scripts.
# Functions using traditional ap/h/hammer approaches.
...
concurrent_execution() {
    ...
    test=49-cleanup-container-host-logs
    skip_measurement=true a "${test}.log" \
        -m ansible.builtin.shell \
        -a 'rm -f /root/out_*.log*' \
        container_hosts

    min_containers_per_batch=4
    initial_concurrent_per_container_host=$(( \
        ((initial_expected_concurrent_registrations / num_container_hosts + min_containers_per_batch - 1) \
        / min_containers_per_batch) * min_containers_per_batch \
    ))
    num_containers_per_container_host="$(get_inventory_var containers_count container_hosts[0])"
    if [[ "$tasks_list" == *registration* ]]; then
        retry_failed="${PARAM_retry_failed:-true}"
    fi
